/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BillingAccountWhereUniqueInput } from "./BillingAccountWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { BillingAccountUpdateInput } from "./BillingAccountUpdateInput";

@ArgsType()
class UpdateBillingAccountArgs {
  @ApiProperty({
    required: true,
    type: () => BillingAccountWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => BillingAccountWhereUniqueInput)
  @Field(() => BillingAccountWhereUniqueInput, { nullable: false })
  where!: BillingAccountWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => BillingAccountUpdateInput,
  })
  @ValidateNested()
  @Type(() => BillingAccountUpdateInput)
  @Field(() => BillingAccountUpdateInput, { nullable: false })
  data!: BillingAccountUpdateInput;
}

export { UpdateBillingAccountArgs as UpdateBillingAccountArgs };
